...
<div>
	<form:form commandName="commentInfoForm"
		action="${pageContext.request.contextPath}/secure/Comments/save"
		method="post">
		...
		<TABLE CLASS="ContentTable">
			...
			<TR>
				...
				<TD>
				    <font color="red"><c:out id="Message" value="${commentInfoForm.errorMessage}"/></font>
					<div class="formBlock boxShadow3 radius10">
						<TABLE>
							<TR>
								<TD><LABEL FOR="">District Code</LABEL></TD>

								<TD><form:select id="districtCodeList"
										path="districtCode" size="1" >
										<form:option value="NONE" label="-- Select --" />
										<form:options items="${districtCodeList}"></form:options>
									</form:select></TD>
								<TD><LABEL FOR="">&nbsp;&nbsp;&nbsp;&nbsp;District Name</LABEL> <form:input id="districtNameField" path=""></form:input>
								</TD>
								<TD><LABEL FOR="">&nbsp;&nbsp;&nbsp;&nbsp;Fiscal Year</LABEL></TD>
								<TD><form:select id="fiscalYearList" path="fiscalYear" size="1" >
										<form:option value="NONE" label="-- Select --" />
										<form:options items="${fiscalYearList}"></form:options>
									</form:select>
								</TD>
								<TD ALIGN="right">&nbsp;&nbsp;&nbsp;&nbsp;Fiscal Month</TD>
								<TD><form:select id="fiscalMonthList" path="fiscalMonth" size="1" >
								    	<form:option value="NONE" label="-- Select --" />
								    	<form:options items="${fiscalMonthList}"></form:options><font color="red"><b>*</b></font>
								    </form:select>
								</TD>
							</TR>
						</TABLE>
						...
						<table id="uploadNotesTable" class="display">
							...
							<tbody>
								<c:forEach var="notesGrid"
									items="${commentInfoForm.commentInfoGrid}">
									<tr>
